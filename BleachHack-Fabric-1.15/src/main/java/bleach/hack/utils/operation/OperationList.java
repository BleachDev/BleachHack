...
public class OperationList {
	...
	public OperationList(Operation... operations) {
		this.operations = new ArrayList<Operation>(Arrays.asList(operations));
	}
	...
	public Box getBox() {
		...
		for (Operation o: operations) {
			if (box == null)
				...
				box = new Box(Math.min(box.x1, o.pos.getX()), Math.min(box.y1, o.pos.getY()), Math.min(box.z1, o.pos.getZ()),
						Math.max(box.x2, o.pos.getX() + 1), Math.max(box.y2, o.pos.getY() + 1), Math.max(box.z2, o.pos.getZ() + 1));
		}
