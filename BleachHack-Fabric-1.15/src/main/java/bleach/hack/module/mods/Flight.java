...
public class Flight extends Module {
	...
	public Flight() {
		super("Flight", GLFW.GLFW_KEY_G, Category.MOVEMENT, "Allows you to fly", new SettingMode("Mode", "Normal", "Static", "Jetpack", "ec.me"),
				new SettingSlider("Speed", 0, 5, 1, 1), new SettingMode("AntiKick", "Off", "Fall", "Bob", "Packet"));
	}
	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asMode().mode == 0) {
			...
			else if (getSetting(2).asMode().mode == 1 && WorldUtils.NONSOLID_BLOCKS.contains(mc.world
					.getBlockState(new BlockPos(mc.player.getPosVector().getX(), mc.player.getPosVector().getY() - 0.069, mc.player.getPosVector().getZ())).getBlock()))
				mc.player.setVelocity(0, mc.player.age % 20 == 0 ? -0.069 : 0, 0);
			else if (getSetting(2).asMode().mode == 2)
				mc.player
						.setVelocity(0,
								mc.player.age % 40 == 0
										? (WorldUtils.NONSOLID_BLOCKS.contains(mc.world.getBlockState(
												new BlockPos(mc.player.getPosVector().getX(), mc.player.getPosVector().getY() + 1.15, mc.player.getPosVector().getZ()))
												.getBlock()) ? 0.15 : 0)
										: mc.player.age % 20 == 0
												? (WorldUtils.NONSOLID_BLOCKS.contains(mc.world.getBlockState(new BlockPos(mc.player.getPosVector().getX(),
														mc.player.getPosVector().getY() - 0.15, mc.player.getPosVector().getZ())).getBlock()) ? -0.15 : 0)
												: 0,
								0);
			...
			if (InputUtil.isKeyPressed(mc.getWindow().getHandle(), InputUtil.fromName(mc.options.keyJump.getName()).getKeyCode())) {
				mc.player.jump();
			} else {
				if (InputUtil.isKeyPressed(mc.getWindow().getHandle(), InputUtil.fromName(mc.options.keySneak.getName()).getKeyCode())) {
