	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asToggle().state) {
			for (Entry<EquipmentSlot, int[]> e: armorMap.entrySet()) {
				ItemStack is = mc.player.inventory.getInvStack(e.getValue()[0]);
				...
				if (is.isDamageable() && is.getMaxDamage() - is.getDamage() < 7) {
					...
					int forceMoveSlot = -1;
					for (int s = 0; s < 36; s++) {
						if (mc.player.inventory.getInvStack(s).isEmpty()) {
							mc.interactionManager.clickSlot(mc.player.container.syncId, armorSlot, 1, SlotActionType.QUICK_MOVE, mc.player);
							return;
						} else if (!(mc.player.inventory.getInvStack(s).getItem() instanceof ToolItem)
								&& !(mc.player.inventory.getInvStack(s).getItem() instanceof ArmorItem)
								&& !(mc.player.inventory.getInvStack(s).getItem() instanceof ElytraItem)
								&& mc.player.inventory.getInvStack(s).getItem() != Items.TOTEM_OF_UNDYING && forceMoveSlot == -1) {
							forceMoveSlot = s;
						}
					}
					...
					if (forceMoveSlot != -1) {
						...
						mc.interactionManager.clickSlot(mc.player.container.syncId,
					...
					mc.interactionManager.clickSlot(mc.player.container.syncId, armorSlot, 1, SlotActionType.THROW, mc.player);
					return;
				}
			}
		}
		...
		for (int s = 0; s < 36; s++) {
			...
			if (prot > 0) {
				EquipmentSlot slot = (mc.player.inventory.getInvStack(s).getItem() instanceof ElytraItem
		...
		for (Entry<EquipmentSlot, int[]> e: armorMap.entrySet()) {
			if (e.getValue()[2] != -1) {
				if (e.getValue()[1] == -1 && e.getValue()[2] < 9) {
					...
					mc.interactionManager.clickSlot(mc.player.container.syncId, newArmorslot, 0, SlotActionType.PICKUP, mc.player);
					mc.interactionManager.clickSlot(mc.player.container.syncId, armorSlot, 0, SlotActionType.PICKUP, mc.player);
