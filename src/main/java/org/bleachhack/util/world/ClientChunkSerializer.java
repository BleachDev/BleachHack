...
public class ClientChunkSerializer {
	...
	public static NbtCompound serialize(ClientWorld world, Chunk chunk) {
		...
		if (blendingData != null) {
			BlendingData.CODEC.encodeStart(NbtOps.INSTANCE, blendingData).result().ifPresent(nbtElement -> nbtCompound.put("blending_data", nbtElement));
		}

		if ((belowZeroRetrogen = chunk.getBelowZeroRetrogen()) != null) {
			BelowZeroRetrogen.CODEC.encodeStart(NbtOps.INSTANCE, belowZeroRetrogen).result().ifPresent(nbtElement -> nbtCompound.put("below_zero_retrogen", nbtElement));
		}
