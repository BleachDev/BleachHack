...
public class Teleport extends Module {
    ...
    public Teleport() {
        super("Teleport", GLFW.GLFW_KEY_V, Category.COMMAND, "What are you doing here?", settings);
    }
    ...
    public void onTick(EventTick event) {
        ...
        if (mc.world.isChunkLoaded(chunkX, chunkZ)) {
            ...
            if (targetVec.distanceTo(mc.player.getPosVector()) < 0.3D || getSettings().get(3).toSlider().getValue() == 0 || mc.player.deathTime == 0) {
                alreadyDone = false;
                getSettings().get(0).toSlider().value = 0.0d;
                getSettings().get(1).toSlider().value = 0.0d;
                getSettings().get(2).toSlider().value = 0.0d;
                getSettings().get(3).toSlider().value = 1.0d;
                BleachHack.eventBus.unregister(this);
            }
            if (targetVec.distanceTo(mc.player.getPosVector()) > getSettings().get(3).toSlider().getValue()) {
                getSettings().get(3).toSlider().value = targetVec.distanceTo(mc.player.getPosVector());
            }
