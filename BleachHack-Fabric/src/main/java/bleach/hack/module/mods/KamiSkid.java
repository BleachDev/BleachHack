		...
		for(Entity e: mc.world.getEntities()) {
			if(e instanceof EnderCrystalEntity && mc.player.distanceTo(e) <= getSettings().get(9).toSlider().getValue()) {
					...
					if (((System.nanoTime() / 1000000) - systemTime) >= 250) {
						if (getSettings().get(8).toToggle().state && mc.player.hasStatusEffect(StatusEffects.WEAKNESS)) {
							if (!isAttacking) {
								...
								oldSlot = Item.getRawId(mc.player.getMainHandStack().getItem());
								isAttacking = true;
							}
							newSlot = -1;
							for (int i = 0; i < 9; i++) {
				...
				entities.addAll(Streams.stream(mc.world.getEntities())
						.filter(g -> (g instanceof PlayerEntity && getSettings().get(3).toToggle().state)
								|| (g instanceof Monster && getSettings().get(4).toToggle().state)
								|| (EntityUtils.isAnimal(e) && getSettings().get(5).toToggle().state))
				...
				if (getSettings().get(6).toToggle()	.state) {
					...
					//mc.player.networkHandler.sendPacket(new PlayerActionC2SPacket(q, f, (offhand ? Hand.OFF_HAND : Hand.MAIN_HAND), Vec3d.ZERO));
				}
