    ...
    public void onCommand(final String command, final String[] args) throws Exception {
        ...
        if (args.length >= 3) {
            try {
                ...
                ModuleManager.getModule(Teleport.class).getSettings().get(0).toSlider().value = x;
                ModuleManager.getModule(Teleport.class).getSettings().get(1).toSlider().value = y;
                ModuleManager.getModule(Teleport.class).getSettings().get(2).toSlider().value = z;
                ModuleManager.getModule(Teleport.class).getSettings().get(3).toSlider().value = blocksPerTeleport;
