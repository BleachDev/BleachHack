...
public class SettingToggle extends SettingBase {
	...
	public void readOldSettings(String settings) {
		if (settings.contains(">") && !children.isEmpty()) {
			settings = settings.replaceFirst(">", ">`");
			String[] split = settings.split(">`");
			
			state = Boolean.parseBoolean(split[0]);
			
			List<String> subSet = getChildren(split[1]);
			
			System.out.println(Arrays.toString(split) + " | " + subSet);
			
			int i = 0;
			for (SettingBase sb: children) {
				if (i >= subSet.size()) break;
				
				sb.readOldSettings(subSet.get(i));
				i++;
			}
		} else {
			state = Boolean.parseBoolean(settings);
		}
	}
