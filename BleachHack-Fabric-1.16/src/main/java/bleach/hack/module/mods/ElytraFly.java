...
public class ElytraFly extends Module {
	...
	public ElytraFly() {
		super("ElytraFly", KEY_UNBOUND, Category.MOVEMENT, "Improves the elytra", new SettingMode("Mode", "Normal", "Control", "Bruh Momentum"),
	...
	public void onClientMove(EventClientMove event) {
		...
		if (getSetting(0).asMode().mode == 1 && mc.player.isFallFlying()) {
			...
			if (!mc.options.keyBack.isPressed() && !mc.options.keyLeft.isPressed() && !mc.options.keyRight.isPressed() && !mc.options.keyForward.isPressed()) {
	...
	public void onTick(EventTick event) {
		Vec3d vec3d = new Vec3d(0, 0, getSetting(1).asSlider().getValue()).rotateX(getSetting(0).asMode().mode == 1 ? 0 : -(float) Math.toRadians(mc.player.pitch))
		...
		if (mc.player.isFallFlying()) {
			if (getSetting(0).asMode().mode == 0 && mc.options.keyForward.isPressed()) {
				mc.player.setVelocity(mc.player.getVelocity().x + vec3d.x + (vec3d.x - mc.player.getVelocity().x),
		...
		} else if (getSetting(0).asMode().mode == 2 && !mc.player.isOnGround() && mc.player.inventory.getArmorStack(2).getItem() == Items.ELYTRA
