		...
		for (Entity e : mc.world.getEntities()) {
			if (e instanceof ChestMinecartEntity && getSetting(7).asToggle().state) {
				...
				if (((ItemFrameEntity) e).getHeldItemStack().getItem() == Items.AIR) {
					...
					int axis = e.getBoundingBox().maxX - e.getBoundingBox().minX < e.getBoundingBox().maxY - e.getBoundingBox().minY ? 0
							: e.getBoundingBox().maxY - e.getBoundingBox().minY < e.getBoundingBox().maxZ - e.getBoundingBox().minZ ? 1 : 2;
