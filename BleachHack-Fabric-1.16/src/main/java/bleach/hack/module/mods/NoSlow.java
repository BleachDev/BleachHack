		...
		if (getSetting(5).asToggle().state && mc.currentScreen != null && !(mc.currentScreen instanceof ChatScreen)) {
			...
			if (getSetting(5).asToggle().asToggle().getChild(0).asToggle().state) {
				mc.options.keySneak.setPressed(
						InputUtil.isKeyPressed(mc.getWindow().getHandle(), InputUtil.fromTranslationKey(mc.options.keySneak.getBoundKeyTranslationKey()).getCode()));
			}
			...
			if (getSetting(5).asToggle().asToggle().getChild(2).asToggle().state) {
				...
				if (getSetting(5).asToggle().asToggle().getChild(2).asToggle().asToggle().getChild(1).asToggle().state) {
					if (yaw == 0f && pitch != 0f)
				...
				if (getSetting(5).asToggle().asToggle().getChild(2).asToggle().asToggle().getChild(0).asToggle().state) {
