...
public class AutoDonkeyDupe extends Module {
	...
	public void onTick(EventTick event) {
		...
		int slots = getSettings().get(0).toSlider().getValue() <= 0 ? getInvSize(mc.player.getVehicle())
				: Math.min((int) getSettings().get(0).toSlider().getValue(), getInvSize(mc.player.getVehicle()));
		...
		boolean instant = getSettings().get(1).toMode().mode == 0;
