...
public class Peek extends Module {
	...
	public void drawScreen(EventRenderTooltip event) {
		...
		if (getSetting(0).asToggle().state) {
			Either<Boolean, List<OrderedText>> either = drawShulkerToolTip(event.getMatrix(), slot, event.getMouseX(), event.getMouseY());
			if (either != null) {
	...
	public Either<Boolean, List<OrderedText>> drawShulkerToolTip(MatrixStack matrices, Slot slot, int mouseX, int mouseY) {
		...
		if (mode == 1) {
			return Either.right(Lists.transform(Arrays.asList(slot.getStack().getName()), Text::asOrderedText));
		} else if (mode == 2) {
	...
	public void drawMapToolTip(MatrixStack matrices, Slot slot, int mouseX, int mouseY) {
		...
		mc.gameRenderer.getMapRenderer().draw(matrices, immediate, mapState, false, 0xf000f0);
		immediate.draw();
