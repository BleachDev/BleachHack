...
public class AutoFarm extends Module {
	...
	public void onTick(EventTick event) {
		...
		for (BlockPos pos: BlockPos.iterateOutwards(new BlockPos(mc.player.getPos().add(0, mc.player.getStandingEyeHeight(), 0)), ceilRange, ceilRange, ceilRange)) {
			if (mc.player.getPos().add(0, mc.player.getStandingEyeHeight(), 0).distanceTo(Vec3d.ofCenter(pos)) > range)
				continue;
			...
			if (plantSetting.state && mc.world.getEntitiesByClass(LivingEntity.class, new Box(pos.up()), Predicates.alwaysTrue()).isEmpty()) {
				...
				if (block instanceof SoulSandBlock && mc.world.isAir(pos.up()) && plantSetting.getChild(2).asToggle().state) {
					int slot = InventoryUtils.getSlot(true, i -> mc.player.inventory.getStack(i).getItem() == Items.NETHER_WART);
			...
			if (bonemealSetting.state) {
				int slot = InventoryUtils.getSlot(true, i -> mc.player.inventory.getStack(i).getItem() == Items.BONE_MEAL);

				if (slot != -1) {
					if ((bonemealSetting.getChild(0).asToggle().state && block instanceof CropBlock && !((CropBlock) block).isMature(state))
							|| (bonemealSetting.getChild(1).asToggle().state && block instanceof StemBlock && state.get(StemBlock.AGE) < 7)
							...
							|| (bonemealSetting.getChild(5).asToggle().state && block instanceof SaplingBlock && canPlaceSapling(pos))) {
