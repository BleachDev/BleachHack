	...
	public void onDrawOverlay(EventDrawOverlay event) {
		...
		if (getSettings().get(0).asToggle().state && !mc.options.debugEnabled) {
			...
			for (String s: lines) {
				...
				DrawableHelper.fill(event.matrix, 0, count*10, mc.textRenderer.getWidth(s)+3+extra, 10+(count*10), 0x70003030);
				DrawableHelper.fill(event.matrix, 0, count*10, extra, 10+(count*10), color);
				DrawableHelper.fill(event.matrix, mc.textRenderer.getWidth(s)+3+extra, (count*10), mc.textRenderer.getWidth(s)+4+extra, 10+(count*10), color);
				if (count + 1 < lines.size()) {
					DrawableHelper.fill(event.matrix, mc.textRenderer.getWidth(lines.get(count + 1))+4+extra, 10+(count*10),
			...
			DrawableHelper.fill(event.matrix, 0, (count*10), mc.textRenderer.getWidth(lines.get(count-1))+4+extra, 1+(count*10), color);
		}
		...
		if (getSettings().get(9).asToggle().state && !mc.options.debugEnabled) {
			DrawableHelper.fill(event.matrix, 0, 3+(count*10), mc.textRenderer.getWidth("Players:")+4, 13+(count*10), 0x40000000);
			...
			for (Entity e: mc.world.getPlayers().stream().sorted(
				...
				DrawableHelper.fill(event.matrix, 0, 3+(count*10), mc.textRenderer.getWidth(text)+4, 13+(count*10), 0x40000000);
