...
public class SettingSlider extends SettingBase {
	...
	public void render(ModuleWindow window, int x, int y, int len) {
		...
		MinecraftClient.getInstance().textRenderer.drawWithShadow(text + ": " + (round == 0 && getValue() > 100 ? Integer.toString((int) getValue()) : getValue()), x + 2,
				y + 2, window.mouseOver(x, y, x + len, y + 12) ? 0xcfc3cf : 0xcfe0cf);
