...
public class RotationSnap extends Module {
	...
	public void onTick(EventTick event) {
		...
		if (getSetting(2).asToggle().state && mc.currentScreen == null) {
			...
			if (InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_LEFT) && !lDown) {
				mc.player.yaw -= ymode == 0 ? 45 : ymode == 1 ? 30 : ymode == 2 ? 15 : 90;
				lDown = true;
			} else if (!InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_LEFT)) {
				lDown = false;
			}

			if (InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_RIGHT) && !rDown) {
				mc.player.yaw += ymode == 0 ? 45 : ymode == 1 ? 30 : ymode == 2 ? 15 : 90;
				rDown = true;
			} else if (!InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_RIGHT)) {
				rDown = false;
			}

			if (InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_UP) && !uDown) {
				mc.player.pitch = MathHelper.clamp(mc.player.pitch - (pmode == 0 ? 45 : pmode == 1 ? 30 : pmode == 2 ? 15 : 90), -90, 90);
				uDown = true;
			} else if (!InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_UP)) {
				uDown = false;
			}

			if (InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_DOWN) && !dDown) {
				mc.player.pitch = MathHelper.clamp(mc.player.pitch + (pmode == 0 ? 45 : pmode == 1 ? 30 : pmode == 2 ? 15 : 90), -90, 90);
				dDown = true;
			} else if (!InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_DOWN)) {
				dDown = false;
			}
		}
