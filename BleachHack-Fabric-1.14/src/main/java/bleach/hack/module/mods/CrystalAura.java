...
public class CrystalAura extends Module {
	...
	private float getExplosionDamage(BlockPos basePos, LivingEntity target) {
		...
		List<Entity> list_1 = mc.world.getEntities((Entity) null, new Box(int_3, int_7, int_9, int_4, int_8, int_10));
		...
		for (int int_11 = 0; int_11 < list_1.size(); ++int_11) {
			...
			if (!entity.isImmuneToExplosion()) {
				...
				if (double_8 <= 1.0D) {
					double double_9 = entity.x - crystalPos.x;
					double double_10 = entity.y + (double)entity .getStandingEyeHeight() - crystalPos.y;
					double double_11 = entity.z - crystalPos.z;
					...
					if (double_12 != 0.0D) {
						...
						toDamage = DamageUtil.getDamageLeft(toDamage, (float) entity.getArmor(),
								(float) entity.getAttributeInstance(EntityAttributes.ARMOR_TOUGHNESS).getValue());
