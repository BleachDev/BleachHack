...
public class Dispenser32k extends Module {
	...
	public void onTick(EventTick event) {
		...
		if (mc.currentScreen instanceof HopperScreen) {
			...
			if (!(gui.getContainer().getSlot(0).getStack().getItem() instanceof AirBlockItem) && active) {
				...
				boolean pull = false;
				for (int i = 40; i >= 32; i--) {
					if (gui.getContainer().getSlot(i).getStack().isEmpty()) {
						slot = i;
						pull = true;
						break;
					}
		...
		if (openedDispenser && dispenserTicks == 0) {
			mc.interactionManager.method_2906(mc.player.container.syncId, 36 + shulker, 0, SlotActionType.QUICK_MOVE, mc.player);
		}
	...
	private boolean placeBlock(BlockPos pos, int slot, boolean rotate, boolean rotateBack) {
		...
		for (Direction d: Direction.values()) {
			...
			if (rightclickableBlocks.contains(neighborBlock)) mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(mc.player, Mode.START_SNEAKING));
			mc.interactionManager.interactBlock(
					mc.player, mc.world, Hand.MAIN_HAND, new BlockHitResult(new Vec3d(pos), d.getOpposite(), pos, false));
			if (rightclickableBlocks.contains(neighborBlock)) mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(mc.player, Mode.STOP_SNEAKING));
