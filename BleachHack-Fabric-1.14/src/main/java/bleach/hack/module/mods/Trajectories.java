...
public class Trajectories extends Module {
			...
			for (Entity e: mc.world.getEntities()) {
				if (e instanceof Projectile) {
					if (!getSetting(3).asToggle().getChild(0).asToggle().state
							&& (e instanceof SnowballEntity || e instanceof ThrownEggEntity || e instanceof ThrownEnderpearlEntity)) {
						continue;
					}

					if (!getSetting(3).asToggle().getChild(1).asToggle().state && e instanceof ThrownExperienceBottleEntity) {
						continue;
					}
		...
		ItemStack hand = isThrowable(thrower.inventory.getMainHandStack().getItem())
				? thrower.inventory.getMainHandStack() : isThrowable(thrower.inventory.offHand.get(0).getItem())
						...
						Vec3d spawnVec = new Vec3d(
								...
								thrower.y + thrower.getStandingEyeHeight() - 0.1000000015,
						...
						if (hand.getItem() instanceof RangedWeaponItem) {
							...
							return e;
						} else if (hand.getItem() instanceof LingeringPotionItem || hand.getItem() instanceof SplashPotionItem) {
	...
	private boolean isThrowable(Item item) {
		return item instanceof RangedWeaponItem
				...
				|| item instanceof LingeringPotionItem || item instanceof SplashPotionItem || item instanceof TridentItem;
	}
	...
	private void initProjectile(Entity e, Entity thrower, float addPitch, float strength) {
		...
		e.setVelocity(velVec.add(thrower.getVelocity().x, thrower.onGround ? 0.0D : thrower.getVelocity().y, thrower.getVelocity().z));
	}
	...
	private Triple<List<Vec3d>, Entity, BlockPos> simulate(Entity e) {
		...
		for (int i = 0; i < 100; i++) {
			...
			List<Entity> entities = mc.world.getEntities((Entity) null, spoofE.getBoundingBox().expand(0.15));
			...
			spoofE.setPosition(spoofE.getPos().x + spoofE.getVelocity().x,
