				...
				if (is.isDamageable() && is.getMaxDamage() - is.getDamage() < 7) {
					...
					for (int s = 0; s < 36; s++) {
						if (mc.player.inventory.getInvStack(s).isEmpty()) {
							mc.interactionManager.method_2906(mc.player.container.syncId, armorSlot, 1, SlotActionType.QUICK_MOVE, mc.player);
							return;
					...
					if (forceMoveSlot != -1) {
						...
						mc.interactionManager.method_2906(mc.player.container.syncId,
					...
					mc.interactionManager.method_2906(mc.player.container.syncId, armorSlot, 1, SlotActionType.THROW, mc.player);
					return;
				}
			}
		}
