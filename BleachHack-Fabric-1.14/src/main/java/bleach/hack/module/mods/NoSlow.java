...
public class NoSlow extends Module {
	...
	public void onClientMove(EventClientMove event) {
		...
		if (getSetting(2).asToggle().state
				&& mc.world.getBlockState(new BlockPos(mc.player.getPos().add(0, -0.01, 0))).getBlock() == Blocks.SLIME_BLOCK && mc.player.onGround) {
			double d = Math.abs(mc.player.getVelocity().y);
			if (d < 0.1D && !mc.player.isSneaking()) {
		...
		if (getSetting(3).asToggle().state && WorldUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB)) {
			...
			mc.player.slowMovement(mc.player.method_16212(), new Vec3d(1.75, 1.75, 1.75));
		}

		/* Berry Bush */
		if (getSetting(4).asToggle().state && WorldUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.SWEET_BERRY_BUSH)) {
			// also scuffed
			mc.player.slowMovement(mc.player.method_16212(), new Vec3d(1.7, 1.7, 1.7));
		}
