	...
	public void onTick(EventTick event) {
		...
		if (getSetting(5).asToggle().state && mc.currentScreen != null && !(mc.currentScreen instanceof ChatScreen)) {
			...
			if (getSetting(5).asToggle().asToggle().getChild(2).asToggle().state) {
				...
				if (InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_LEFT)) yaw -= 0.5f;
				if (InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_RIGHT)) yaw += 0.5f;
				if (InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_UP)) pitch -= 0.5f;
				if (InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_DOWN)) pitch += 0.5f;
