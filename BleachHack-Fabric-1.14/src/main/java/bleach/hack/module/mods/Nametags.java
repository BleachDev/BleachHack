...
public class Nametags extends Module {
	...
	public void onDisable() {
		...
		for (Entity e: mc.world.getEntities())
			e.setCustomNameVisible(e.hasCustomName());
	...
	public void onEntityLabelRender(EventEntityRender.Label event) {
		if ((EntityUtils.isAnimal(event.getEntity()) && getSetting(3).asToggle().state)
			...
			event.getEntity().setCustomNameVisible(event.getEntity().hasCustomName());
	...
	private Vec3d getRenderPos(Entity e) {
		return mc.currentScreen != null && mc.currentScreen.isPauseScreen() ? e.getPos().add(0, e.getHeight(), 0)
				: new Vec3d(
						e.lastRenderX + (e.x - e.lastRenderX) * mc.getTickDelta(),
						(e.lastRenderY + (e.y - e.lastRenderY) * mc.getTickDelta()) + e.getHeight(),
						e.lastRenderZ + (e.z - e.lastRenderZ) * mc.getTickDelta());
	}

	private String getHealthText(LivingEntity e) {
		if (getSetting(1).asMode().mode == 0) {
			return Formatting.GREEN + "[" + getHealthColor(e) + (int) (e.getHealth() + e.getAbsorptionAmount()) + Formatting.GREEN + "/" + (int) e.getMaximumHealth() + "]";
			...
			if (e.getAbsorptionAmount() - (e.getMaximumHealth() - e.getHealth()) > 0) {
				health += Formatting.YELLOW + " +" + (int) (e.getAbsorptionAmount() - (e.getMaximumHealth() - e.getHealth()));
			}

			return health;
		} else {
			return getHealthColor(e) + "[" + (int) ((e.getHealth() + e.getAbsorptionAmount()) / e.getMaximumHealth() * 100) + "%]";
		}
	}
