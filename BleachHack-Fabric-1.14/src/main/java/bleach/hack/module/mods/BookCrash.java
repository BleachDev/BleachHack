...
public class BookCrash extends Module {
	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asMode().mode == 2) {
			// Text text = Text.Serializer.fromJson("{\"text\":\"" + size +
			// "\",\"color\":\"red\"}");
			Text text = Text.Serializer.fromJson("{\"text\":\"pp\"}");
			Random rand = new Random();
			mc.player.networkHandler
					.sendPacket(new UpdateSignC2SPacket(new BlockPos(rand.nextInt(29999999), rand.nextInt(29999999), rand.nextInt(29999999)), text, text, text, text));
		} else {
