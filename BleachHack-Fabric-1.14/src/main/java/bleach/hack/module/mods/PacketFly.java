	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asMode().mode == 0) {
			...
			if (mc.player.headYaw != mc.player.yaw) {
				mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.LookOnly(mc.player.headYaw, mc.player.pitch, mc.player.onGround));
				return;
			}
			...
			mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionOnly(mc.player.x + mX, mc.player.y + mY, mc.player.z + mZ, false));
			mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionOnly(mc.player.x + mX, mc.player.y - 420.69, mc.player.z + mZ, true));
