...
public class ClickTp extends Module {
	...
	public void onTick(EventTick event) {
		if (InputUtil.isKeyPressed(mc.window.getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
			pos = null;
			dir = null;
			return;
		}
		...
		if (pos != null && dir != null) {
			if (GLFW.glfwGetMouseButton(mc.window.getHandle(), GLFW.GLFW_MOUSE_BUTTON_RIGHT) == 1 && mc.currentScreen == null && !antiSpamClick) {
				antiSpamClick = true;

				Vec3d tpPos = new Vec3d(pos.offset(dir, dir == Direction.DOWN ? 2 : 1)).add(0.5, 0, 0.5);

				if (getSetting(2).asToggle().state) {
					mc.player.updatePosition(mc.player.x, tpPos.y, mc.player.z);
					mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionOnly(mc.player.x, tpPos.y, mc.player.z, false));
				}

				mc.player.updatePosition(tpPos.x, tpPos.y, tpPos.z);
				mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionOnly(tpPos.x, tpPos.y, tpPos.z, false));
			} else if (GLFW.glfwGetMouseButton(mc.window.getHandle(), GLFW.GLFW_MOUSE_BUTTON_RIGHT) == 0) {
				antiSpamClick = false;
			}
		}
	}
