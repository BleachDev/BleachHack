	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asMode().mode == 2) {
			if (placedWater && mc.player.fallDistance == 0) {
				int slot = InventoryUtils.getSlot(true, i -> mc.player.getInventory().getStack(i <= 9 || i == 40 ? i : 0).getItem() == Items.BUCKET);
				...
				int slot = InventoryUtils.getSlot(true, i -> mc.player.getInventory().getStack(i <= 9 || i == 40 ? i : 0).getItem() == Items.WATER_BUCKET);
