...
public class AutoThrow extends Module {
	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asToggle().state) {
			int slot = InventoryUtils.getSlot(true, i -> mc.player.getInventory().getStack(i).getItem() == Items.EXPERIENCE_BOTTLE);
		...
		if (getSetting(1).asToggle().state) {
			int slot = InventoryUtils.getSlot(true, i -> {
				if (mc.player.getInventory().getStack(i).getItem() == Items.SPLASH_POTION) {
					Potion potion = PotionUtil.getPotion(mc.player.getInventory().getStack(i));
			...
			if (slot != 1) {
				PotionUtil.getPotion(mc.player.getInventory().getStack(slot)).getEffects().forEach(s -> effectCache.put(s.getEffectType(), 100));
				use(slot, getSetting(2).asToggle().state);
				return;
			}
		}
	...
	private void use(int slot, boolean switchback) {
		if (slot != -1) {
			int prevSlot = slot < 9 && slot != mc.player.getInventory().selectedSlot ? mc.player.getInventory().selectedSlot : -1;
